...
const DetailedStockData = memo(function DetailedStockData({ symbol, price }) {
    ...
    let stockClasses = clsx( "flex", "flex-col", "w-1/2",
        {"hidden": !isVisible},
    );
    let buttonClasses = clsx("text-indigo-100", "bg-indigo-500", "hover:bg-indigo-600", "transition", "duration-250", "ease-in-out", "px-1", "mt-2",
        {"hidden": !isVisible},
    );
    //Response Message classes
    let errorClasses = clsx( "flex", "flex-row", "justify-center", "mt-3", "pt-1", "text-red-600", "text-xl",
        {"hidden": !isMessageVisible || !errorMessage},
    );
    let successClasses = clsx( "flex", "flex-row", "justify-center", "mt-3", "pt-1" ,"text-green-600", "text-xl",
    ...
    return (
        <>
            ...
            <div className="flex flex-row justify-center">
                <PrimaryButton className={buttonClasses} onClick={() => handleBookmark(price, details, setErrorMessage, setSuccessMessage, setIsMessageVisible)}>
                    Bookmark this stock!
                </PrimaryButton>
            </div>
